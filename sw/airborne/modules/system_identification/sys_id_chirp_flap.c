...
static struct FirstOrderLowPass filter;

// Chirp and noise values for all axes (indices correspond to the axes given in CHIRP_AXES)
static pprz_t current_chirp_value;

static void set_current_chirp_value(void)
{
  if (chirp_active) {
#if CHIRP_USE_NOISE

    float amplitude, noise;
    noise = update_first_order_low_pass(&filter, rand_gaussian());
    amplitude = chirp_noise_stdv_onaxis_ratio * chirp_amplitude;
    current_chirp_value = (int32_t)(noise * amplitude);
    

#endif

    current_chirp_value += (int32_t)(chirp_amplitude * chirp.current_value);
  } else {
    
    current_chirp_value = 0;
...
void sys_id_chirp_init(void)
{
  ...
  init_first_order_low_pass(&filter, tau, SYS_ID_CHIRP_RUN_PERIOD, 0);
  current_chirp_value = 0;
}
